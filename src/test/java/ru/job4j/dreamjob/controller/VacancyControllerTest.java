...
class VacancyControllerTest {
    ...
    @Test
    public void whenVacancyControllerGetByIdThanSuccessfully() {
        var vacancy = new Vacancy(1, "vac1", "desc11", now(), true, 1, 2);
        Optional<Vacancy> result = Optional.of(vacancy);
        var city1 = new City(1, "Москва");
        var city2 = new City(2, "Санкт-Петербург");
        var expectedCities = List.of(city1, city2);
        when(vacancyService.findById(vacancy.getId())).thenReturn(Optional.of(result.get()));
        when(cityService.findAll()).thenReturn(expectedCities);
    ...
    @Test
    public void whenVacancyControllerGetByIdThanFail() {
        var vacancy = new Vacancy(1, "vac1", "desc11", now(), true, 1, 2);
        Optional<Vacancy> result = Optional.empty();
        when(vacancyService.findById(vacancy.getId() + 1)).thenReturn(result);
    ...
    @Test
    public void whenVacancyControllerUpdateThanSuccessfully() throws IOException {
        var vacancy = new Vacancy(1, "vac1", "desc11", now(), true, 1, 2);
        var fileDto = new FileDto(testFile.getOriginalFilename(), testFile.getBytes());
        var vacancyArgumentCaptor = ArgumentCaptor.forClass(Vacancy.class);
        var fileDtoArgumentCaptor = ArgumentCaptor.forClass(FileDto.class);
    ...
    @Test
    public void whenVacancyControllerDeleteThanSuccessfully() {
        var vacancy = new Vacancy(1, "vac1", "desc11", now(), true, 1, 2);
        Optional<Vacancy> result = Optional.of(vacancy);
        when(vacancyService.findById(vacancy.getId())).thenReturn(Optional.of(result.get()));
